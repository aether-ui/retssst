  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Transform Printful data to match our Product type
        const transformedProducts = data.result.map((item: any) => ({
          ...
          imageUrls: [item.thumbnail_url],
          category: item.type_name.toLowerCase(),
          tags: [],
          sizes: item.variants.map((variant: any) => variant.size || 'One Size'),
          colors: item.variants.map((variant: any) => ({
